...
\item In order for processes to run Erlang code linearly whilst users can only run Erlang code asynchronously, processes must bind a reference to themselves whenever they call a function which on some JavaScript execution engines causing extremely severe performance issues.
\item Ports as a datatype aren't implemented but could be implement to help facilitate distributed programming.
